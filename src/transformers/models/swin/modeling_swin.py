...
class SwinLayer(nn.Module):
    ...
    def forward(
        ...
        if self.rank is not None:
            ...
            # 패딩 처리 확인
            if self.rank == 0:
                for row in range(attention_output.shape[1], 49): 
                    count = torch.isnan(padded_tensor[:, row, :]).sum().item() 
                    if count != attention_output.shape[0] * attention_output.shape[2]:
                        print("❌ Incorrect padding")
            else:
                for row in range(49 - attention_output.shape[1]): 
                    count = torch.isnan(padded_tensor[:, row, :]).sum().item() 
                    if count != attention_output.shape[0] * attention_output.shape[2]:
                        print("❌ Incorrect padding")

            attention_output = padded_tensor
        ...
        # if self.rank is not None:
        #     nan_count_list = []
        #     num_rows = shifted_windows.shape[1]
        #     for row in range(num_rows):  # 56개의 행에 대해 반복
        #         count = torch.isnan(shifted_windows[:, row, :, 0]).sum().item()  # 해당 행에서 NaN 개수를 셈
        #         nan_count_list.append(count)

        #     # 결과 출력
        #     for i, count in enumerate(nan_count_list):
        #         print(f"Row {i}: NaN 개수 = {count}")
        ...
        # if self.rank is not None:
        #     nan_count_list = []
        #     num_rows = attention_windows.shape[1]
        #     for row in range(num_rows): 
        #         count = torch.isnan(attention_windows[:, row, :, 0]).sum().item() 
        #         nan_count_list.append(count)
        ...
        if self.rank is not None:
            # 행에 따라서 분할이 되어 있는지 확인 
            num_rows = attention_windows.shape[1]
            for row in range(num_rows): 
                count = torch.isnan(attention_windows[:, row, :]).sum().item() 
                if count not in [0, attention_windows.shape[0] * attention_windows.shape[2]]:
                    print("⚠️ There are mixed None values and valid values within the row.")
...
class SwinModel(SwinPreTrainedModel):
    ...
    def init(self, rank):
        self.encoder = SwinEncoder(self.config, self.embeddings.patch_grid, rank=rank)
